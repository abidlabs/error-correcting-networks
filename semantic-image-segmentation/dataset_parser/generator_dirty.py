...
def get_result_map(b_size, y_img, ignore_pedestrians):
...
def data_generator(d_path, b_size, mode, dirty=True, ignore_pedestrians=False):
    data = h5py.File(d_path, 'r')
    if mode == 'train' and dirty:
        x_imgs = data.get('/' + mode + '_dirty/x')
        y_imgs = data.get('/' + mode + '_dirty/y')
    elif mode == 'train' and not(dirty):
        x_imgs = data.get('/' + mode + '_clean/x')
        y_imgs = data.get('/' + mode + '_clean/y')
    else:
    ...
    y = []
    while True:
        random.seed(0)  # to align coarse and fine labels.
        ...
        for i in range(d_size):
            ...
            if len(x) == b_size:
                ...
                yield x_result, get_result_map(b_size, y_result, ignore_pedestrians)
