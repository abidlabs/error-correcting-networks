...
def get_result_map(b_size, y_img, ignore_pedestrians, misclassified_frac):
    y_img = np.squeeze(y_img, axis=3)
    person = (y_img == 24)
    car = (y_img == 26)
    road = (y_img == 7)
    
    if ignore_pedestrians:
        result_map = np.zeros((b_size, 256, 512, 3))        
        background = np.logical_not(car + road)
        for b in range(b_size):
            result_map[b, :, :, 0] = np.where(background[b], 1, 0)
            if random.random() > misclassified_frac:
#                 print('not miscl')
                result_map[b, :, :, 1] = np.where(car[b], 1, 0)
                result_map[b, :, :, 2] = np.where(road[b], 1, 0)
            else:
#                 print('miscl')
                result_map[b, :, :, 2] = np.where(car[b] | road[b], 1, 0)  # label the car as road instead
...
def data_generator(d_path, b_size, mode, ignore_pedestrians=False, misclassified_frac=0.5):
    ...
    while True:
        ...
        for i in range(d_size):
            ...
            if len(x) == b_size:
                ...
                _ = np.zeros(b_size)
                seed = 0 # random.randrange(1, 1000)
                ...
                yield x_result, get_result_map(b_size, y_result, ignore_pedestrians, misclassified_frac)
