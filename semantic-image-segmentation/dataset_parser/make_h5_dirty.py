...
def write_data(h5py_file, mode, x_paths, y_paths):
    ...
    if mode == 'train':
        group_clean = h5py_file.create_group('train_clean')
        group_dirty = h5py_file.create_group('train_dirty')
        
        dirty_indices = np.random.permutation(num_data)[:int(FRAC_DIRTY*num_data)]
        num_dirty = len(dirty_indices)
        num_clean = num_data - num_dirty
        
        x_dset_c = group_clean.create_dataset('x', shape=(num_clean, ), dtype=uint8_dt)
        y_dset_c = group_clean.create_dataset('y', shape=(num_clean, ), dtype=uint8_dt)
        x_dset_d = group_dirty.create_dataset('x', shape=(num_dirty, ), dtype=uint8_dt)
        y_dset_d = group_dirty.create_dataset('y', shape=(num_dirty, ), dtype=uint8_dt)
        count_c = 0
        count_d = 0
        ...
        for i in range(num_data):
            ...
            if i in dirty_indices:
                y_img[:DIRTY_WIDTH, :] = 3
                y_img[-DIRTY_WIDTH:, :] = 3                
                x_dset_d[count_d] = x_img.flatten()
                y_dset_d[count_d] = y_img.flatten()
                count_d += 1
            else:
                x_dset_c[count_c] = x_img.flatten()
                y_dset_c[count_c] = y_img.flatten()
                count_c += 1
...
def make_h5py():
    ...
    h5py_file = h5py.File(os.path.join(dir_path, 'data-dirty.h5'), 'w')
